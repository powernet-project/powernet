...
class HardwareRPi:
    def __init__(self, gpio_map=None, N_SAMPLES=100):
        ...
        self.app_orig_states = ["OFF", "OFF", "OFF", "ON", "OFF", "OFF", "OFF",
                                "OFF"]  # Battery included, switch included
        self.app_new_status = ["OFF", "OFF", "OFF", "ON", "OFF", "OFF", "OFF",
                               "OFF"]  # Battery included, switch included
        ...
        self.input_sources_statesDB = {'AC1': [3, 5], 'SE1': [4, 12], 'RF1': [6, 10], 'DW1': [7, 14], 'WM1': [8, 29],
                                       'CW1': [9, 13], 'PW2': [10, 19], 'PV': [11, 30], 'Mains1': [12, 31],
                                       'Mains2': [13, 32]}
        self.sourcesDBID = [self.input_sources_statesDB['AC1'][0], self.input_sources_statesDB['SE1'][0],
                            self.input_sources_statesDB['RF1'][0], self.input_sources_statesDB['CW1'][0],
                            self.input_sources_statesDB['DW1'][0], self.input_sources_statesDB['WM1'][0],
                            self.input_sources_statesDB['PW2'][0]]
    ...
    def dbWriteMeasurements(self, vals):
        try:
            ...
            c = conn.cursor()
            # print vals
            c.execute("INSERT INTO measurements (rms, currentdate, currenttime, source_id) VALUES ((?), (?), (?), (?))",
    ...
    def dbWriteStates(self, vals):
        try:
            ...
            # print vals
            c.execute(
                "INSERT INTO input_sources_state (state, currentdate, currenttime, source_id) VALUES ((?), (?), (?), (?))",
                (vals[0], vals[1], vals[2], vals[3]))
            # print 'State written...'
            # print 'Vals: ', vals
        except sqlite3.IntegrityError:
                ...
                try:
                    ...
                    if len(d_fb[1]["samples"]) == 10:
                        try:
                            ...
                            r_post_rms = requests.post(self.PWRNET_API_BASE_URL + "rms/", json={'devices_json': d_fb},
    ...
    def devices_th(self, q_batt):
        ...
        while True:
            try:
                dev_status = requests.get(self.PWRNET_API_BASE_URL + "device", timeout=self.REQUEST_TIMEOUT).json()[
                    "results"]
