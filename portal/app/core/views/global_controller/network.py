...
class Network(object):
    ...
    def __init__(self, storagePen, solarPen, nodesPen, pDemandFull, rDemandFull, pricesFull, root, Ybus, startIdx=0,
                 sNormFull=np.nan, Vmin=0.95, Vmax=1.05, Vtol=0, v_root=1.022, random=True, rampUAll={}):
        ...
        if random == True:
            self.netDemandFull, sGenFull, self.battnodes, self.qmin, self.qmax, self.umin, self.umax = self.setStorageSolar(
                pDemandFull, sNormFull, storagePen, solarPen, nodesPen, root)
            self.makeNetwork(Ybus, root, self.battnodes, self.qmin, self.qmax, self.umin, self.umax, Vmin, Vmax, Vtol,
                             v_root)
        else:
    ...
    def inputStorage(self, Ybus, netDemandFull, battnodes, qmin, qmax, umin, umax):
        ...
        self.makeNetwork(Ybus, self.root, battnodes, qmin, qmax, umin, umax, self.Vmin, self.Vmax, self.Vtol,
                         self.v_root)
