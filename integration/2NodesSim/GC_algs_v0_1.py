...
class Global_Controller(object):
	...
	def Distribute_Ramp(self, Uramp, Ubound_max, Ubound_min, qmaxo, qmino, umax, umin, q0=np.nan):
		...
		obj = Minimize( norm(pos(U - (Ubound_max-tol)) + pos((Ubound_min+tol) - U), 'fro') + norm(U, 1) + ramp_weight*norm(sum_entries(U, axis=0) - Uramp) )
	...
	def disaggregateRampSignal(self, t_idx, q0, ramp_next, t_idx_old):
		#After receiving a ramp signal, disaggregate the ramp accross the storage nodes
		#t_idx_old is coming from LC_GLD after separating LC from GC
		...
		Rtimes = rampObj.times - t_idx_old
	...
	def disaggregateRampSignal_mod(self, t_idx, q0, ramp_next, t_idx_old,uboundMin,uboundMax):
		#After receiving a ramp signal, disaggregate the ramp accross the storage nodes
		#t_idx_old is coming from LC_GLD after separating LC from GC
		...
		#print('q0: ', Qall[:,ramp_next])
		#U_ramp_test, UboundFlag = self.Distribute_Ramp(rampObj.mag, self.Ubound_max[:,Rtimes], self.Ubound_min[:,Rtimes], qmax, qmin, umax, umin, q0)
		U_ramp_test, UboundFlag = self.Distribute_Ramp(rampObj.mag, uboundMax[:,Rtimes], uboundMin[:,Rtimes], qmax, qmin, umax, umin, q0)
		if UboundFlag == 1:
			self.Ubounds_vio.append(ramp_next)
		if np.any(np.isnan(U_ramp_test)):
			print('True ramp is infeasible do not follow')
